...
gulp.task('jade', function() {
  // var YOUR_LOCALS = {};
  var all = gulp.src(sources.jadeView)
    .pipe(plumber({
      errorHandler: onError
    }))
    ...
    .pipe(gulp.dest(destinations.htmlView));

  var index = gulp.src('./src/*.jade')
    .pipe(plumber({
      errorHandler: onError
    }))
    .pipe(jade({
...
gulp.task('watch', function() {
  ...
  gulp.watch(sources.jadeView, ['jade']);
});
